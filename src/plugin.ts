...
export default function revealRemarkMarkdownPlugin() {
   ...
   */
  function getMarkdownFromSlide(section: HTMLElement) {
   ...
   */
  function getForwardedAttributes(section: HTMLElement) {
   ...
   */
  function getSlidifyOptions(options: Options): Options {
   ...
   */
  function createMarkdownSlide(content: string, options: Options) {
   ...
   */
  function slidify(markdown: string, options: Options) {
    options = getSlidifyOptions(options);
    ...
    (wasHorizontal
      ? sectionStack
      : (sectionStack[sectionStack.length - 1] as string[])
    ...
    for (let i = 0, len = sectionStack.length; i < len; i++) {
      ...
      if (Array.isArray(section)) {
        ...
        const sectionContent = `<section ${
          options.attributes
        }>${childSectionContents.join('')}</section>`;
   ...
   */
  async function processSlides(scope: HTMLElement) {
      ...
      if (section.getAttribute('data-markdown')?.length) {
        try {
          ...
          if (Array.isArray(reason)) {
            const [xhr, url] = reason as [XMLHttpRequest, string];
            section.outerHTML =
  ...
  function loadExternalMarkdown(section: HTMLElement) {
    return new Promise<[XMLHttpRequest, string]>(function (
      this: object,
      resolve,
      reject
    ) {
  ...
  function addAttributes(
    ...
    separatorElementAttributes: string | null,
    separatorSectionAttributes: string | null
  ) {
    if (
      element !== null &&
      ...
      for (let i = 0; i < element.childNodes.length; i++) {
        ...
        if (i > 0) {
          let j = i - 1;
          while (j >= 0) {
            const aPreviousChildElement = element.childNodes[j] as HTMLElement;
            if (
  ...
  function convertSlides() {
    ...
    [...(sections ?? [])].forEach((section) => {
      ...
      const notes = section.querySelector<HTMLElement>('aside.notes');
      ...
      addAttributes(
        ...
        section.getAttribute('data-element-attributes') ||
          (section.parentNode! as HTMLElement).getAttribute(
